...
PKG_URL="https://ftp.gnu.org/gnu/mpc/${PKG_TARBALL}"
...
run_tests() {
    # Habilite com RUN_MPC_TESTS=1
    if [[ "${RUN_MPC_TESTS:-0}" = "1" ]]; then
        log "Executando suíte de testes de ${PKG_NAME}"
        make check || error "Testes do MPC falharam"
    else
        log "RUN_MPC_TESTS!=1 – pulando testes de ${PKG_NAME}"
    fi
}
